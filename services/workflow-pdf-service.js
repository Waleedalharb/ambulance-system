/**
 * خدمة توليد PDF لسير العمل — W-3
 * ═══════════════════════════════════════════════════════════
 * وثيقة تشغيلية A4 تُولَّد عند الاعتماد فقط (قبل قفل الصف) — وثيقة منصة
 * إدارة العمليات، وليست وثيقة صادرة عن هيئة الهلال الأحمر السعودي.
 * تفويض المالك (2026-08-08): إزالة عناصر الإيحاء بالصدور الرسمي عن الهيئة
 * من قوالب PDF — التصميم كما هو (نفس الألوان/الخطوط/الجداول/المواضع).
...
function generateWorkflowPdf(workflow) {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            // ═══ ترويسة المنصة (نفس المواضع والارتفاعات السابقة حرفيًا) ═══
            ...
            // وارتفاع الشريط (68) وكل الألوان/الخطوط بلا أي تغيير بصري آخر.
            try {
            ...
            tr('منصة إدارة العمليات الإسعافية', M, y + 2, CW - 70);
            doc.font('amiri').fontSize(13).fillColor(BLUE);
            tr('قطاع جنوب الرياض', M, y + 16, CW - 70);
            doc.font('amiri').fontSize(11.5).fillColor(BLUE);
            tr('سير العمل الرسمي لبداية المناوبة', M, y + 48, CW - 70);
            y += 68;

            // شارة «وثيقة تشغيلية مقفلة» + المرجع والإصدار
            // تفويض المالك (2026-08-08): «وثيقة رسمية معتمدة» → «وثيقة تشغيلية
            // مقفلة» (بلا أي إيحاء باعتماد رسمي — نفس الشريط/اللون/الموضع).
            doc.roundedRect(M, y, CW, 26, 5).fill('#dcfce7');
            doc.font('amiri').fontSize(10).fillColor('#16a34a');
            tr('وثيقة تشغيلية مقفلة', M + 8, y + 7, 130);
            ...
            // تفويض المالك (2026-08-08): عنوان القسم «الختم والاعتماد» → «الإعداد
            // والاعتماد» (بلا أي إيحاء بختم رسمي — نفس الصندوق/الأعمدة/QR/البصمة).
            need(122);
            y += 8;
            const sealH = 106;
            doc.roundedRect(M, y, CW, sealH, 6).lineWidth(1).strokeColor(BLUE).stroke();
            doc.font('amiri').fontSize(10).fillColor(BLUE);
            tr('الإعداد والاعتماد', M + 10, y + 8, CW - 20);
            ...
            y += 11;
            // تفويض المالك (2026-08-08): «وثيقة معتمدة ومقفلة» → «وثيقة تشغيلية مقفلة».
            tr('وثيقة تشغيلية مقفلة – أي تعديل يتطلب إعادة إصدار موثقة • ' + refNo, M, y, CW);
