...
//  /api/shift-roster/employee-schedule/:id — قراءة فقط، ورابط
//  لسجل تدقيق ذلك الموظف. الخادم يحجب من لا يملك schedule.view
...
struct ScheduleEmployeeView: View {
    ...
    // MARK: - جدول الموظف للشهر المختار
                        ...
                        ForEach(schedule) { day in
                            HStack {
                                ...
                                Text(day.teamName ?? "—")
                                    .font(.caption)
                                    .foregroundStyle(EMSTheme.Colors.textMuted)
                                EMSStatusPill(text: day.shiftName ?? day.shiftCode ?? "—", tone: .action)
                            }
                        }
                        if let id = person.id {
                            NavigationLink {
