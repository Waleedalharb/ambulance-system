...
struct OperationsHomeView: View {
    private enum OpsModule: Hashable, Identifiable {
        case teams, readiness, completion, vehicles, events, decisionCenter, map, positioning, dispatch, forms, workflow, archive, lifecycle, assets, hospitals
        var id: Self { self }
    ...
    private let modules: [OpsModule] = [.lifecycle, .dispatch, .teams, .readiness, .completion, .vehicles, .events, .decisionCenter, .map, .positioning, .forms, .workflow, .archive, .assets, .hospitals]
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            AppLogger.ui.info("OperationsHomeView LIVE-DATA build appeared — 15 modules wired to Backend APIs")
            #endif
        }
